...
public class DatasetGenerator<T> {
    ...
    private DataSet createARecord(int i) {
        ...
        this.assertionsBuilder.startRecord(i);
        ...
        final Consumer<T> expected = this.assertionsBuilder.endRecord(i, rec);
        return new DataSet<T>(rec, expected);
    }
    ...
    private <U> void fullData(int indice, BiConsumer<Schema.Entry, U> builder, String fieldName, boolean isNull,
        ...
        if (isNull) {
            this.assertionsBuilder.addField(indice, field, null);
        } else {
            final U value = valueGetter.get();
            if (value != null) {
                this.assertionsBuilder.addField(indice, field, value);
