                    ...
                    steps {
                        container('main') {
                            withCredentials([sonarCredentials]) {
                                sh "mvn -Dsonar.host.url=https://sonar-eks.datapwn.com -Dsonar.login='${SONAR_LOGIN}' -Dsonar.password='${SONAR_PASSWORD}' -Dsonar.coverage.jacoco.xmlReportPaths=./target/coverage-reports/aggregate.exec sonar:sonar -PITs -s .jenkins/settings.xml -Dtalend.maven.decrypter.m2.location=${env.WORKSPACE}/.jenkins/"
                            }
                        }
                    }
                }
